...
export async function validateDocuments(files: LocalFile[]): Promise<ValidationResult> {
  ...
  const validationResults = await mapLimit(jsonFiles, 5, async (jsonFile) => validateJsonFile(jsonFile, markdownPaths));
...
async function validateJsonFile(jsonFile:LocalFile, markdownPaths:Set<string>): Promise<ValidationError[]> {
