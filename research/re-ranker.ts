...
async function reRankChunks(userQuery: string, retrievedChunks: { content: string }[]) {
  const ranker = await pipeline('text-classification', 'cross-encoder/ms-marco-MiniLM-L-6-v2');
